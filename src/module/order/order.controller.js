...
class OrderController {
  async checkOut(req, res, next) {
    try {
      ...
      const totalValue = orderSvc.calculateTotal(
        cartInfo,
        appliedVoucherDetail,
      );

      //order process mapping
      const orderDetail = await orderSvc.transformToOrder(
        loggedInUser,
        totalValue,
      );
      //order to database
      ...
      //transaction
      let transaction = transactionSvc.transformToTransaction(order);
      await transactionSvc.createTransaction(transaction);

      //email notification
      await orderNtfy.sendOrderDetailNotification(order, orderDetail);
