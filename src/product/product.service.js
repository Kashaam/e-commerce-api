...
class ProductService extends BaseService {
        ...
        try{
            ...
            if(req.files){
                ...
                uploadStatus.map((cloudinaryUploadSuccess)=>{
                    if(cloudinaryUploadSuccess === "fulfilled"){
                        data.images.push(cloudinaryUploadSuccess.values);
                    }
                })
            }
    ...
    publicProductData = (product)=>{
        return {
            ...
            category: product.category && product.category.map((cat)=>(categorySvc.publicCategoryData(cat))),
    ...
    transformUpdateProduct = async(req, oldData)=>{
        try{
            const data = req.body;
            const updatedBy = req.loggedInUser_id;
            ...
            if(data.files){
                ...
                statusResponse.map((cloudinarySuccess)=>{
                    if(cloudinarySuccess === "fulfilled"){
                        data.images.push(cloudinarySuccess.values);
                    }
                })
            }
